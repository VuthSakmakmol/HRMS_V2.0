...
import { fetchHrDashboardTargetLookups } from "../services/hrDashboardTarget.api.js";
...
const targetScopeOptions = computed(() => [
  { label: t("hrDashboardTarget.scopes.overall"), value: "OVERALL" },
  { label: t("hrDashboardTarget.scopes.employeeType"), value: "EMPLOYEE_TYPE" },
]);

const targetScopeFilterOptions = computed(() => [
  { label: t("hrDashboardTarget.allScopes"), value: "" },
  ...targetScopeOptions.value,
]);
...
const selectedFormEmployeeType = computed(
  () =>
    employeeTypes.value.find((item) => item.id === form.employeeTypeId) || null,
);
const formChildOptions = computed(() =>
  (selectedFormEmployeeType.value?.children || []).map((child) => ({
    label: `${child.code} - ${child.name}`,
    value: child.id,
  })),
);
const formRequiresChild = computed(() => formChildOptions.value.length > 0);
const formUsesEmployeeType = computed(
  () => form.targetScope === "EMPLOYEE_TYPE",
);
const selectedFilterEmployeeType = computed(
  () =>
    employeeTypes.value.find((item) => item.id === filters.employeeTypeId) ||
    null,
);
const childFilterOptions = computed(() => [
  { label: t("hrDashboardTarget.allEmployeeTypeChildren"), value: "" },
  ...(selectedFilterEmployeeType.value?.children || []).map((child) => ({
    label: `${child.code} - ${child.name}`,
    value: child.id,
  })),
]);
...
const columns = computed(() => [
  ...
  {
    field: "targetScope",
    header: t("hrDashboardTarget.targetScope"),
    width: "11rem",
    minWidth: "11rem",
  },
  {
    field: "employeeType",
    header: t("hrDashboardTarget.employeeType"),
    width: "12rem",
    minWidth: "12rem",
  },
  {
    field: "employeeTypeChild",
    header: t("hrDashboardTarget.employeeTypeChild"),
    width: "12rem",
    minWidth: "12rem",
  },
  {
...
const formValid = computed(
  () =>
    workspace.ready &&
    Boolean(form.metric) &&
    Boolean(form.targetScope) &&
    (!formUsesEmployeeType.value || Boolean(form.employeeTypeId)) &&
    (!formUsesEmployeeType.value ||
      !formRequiresChild.value ||
      Boolean(form.employeeTypeChildId)) &&
...
function metricLabel(value) {
  return (
    metricOptions.value.find((item) => item.value === value)?.label || value
  );
}
...
async function openEdit(row) {
  ...
  Object.assign(form, emptyForm(), {
    ...
    targetScope:
      row.targetScope || (row.employeeTypeId ? "EMPLOYEE_TYPE" : "OVERALL"),
...
function payload() {
  return {
    ...
    employeeTypeId: formUsesEmployeeType.value ? form.employeeTypeId : null,
    employeeTypeChildId:
      formUsesEmployeeType.value && formRequiresChild.value
  ...
  return [
    {
      ...
      icon: "pi pi-pencil",
      visible:
        row.status !== "ARCHIVED" && auth.hasPermission(PERMISSIONS.UPDATE),
    ...
    },
    {
      ...
      icon: "pi pi-box",
      visible:
        row.status !== "ARCHIVED" && auth.hasPermission(PERMISSIONS.ARCHIVE),
...
watch(
  () => form.targetScope,
  (next, previous) => {
    if (!hydratingForm.value && next !== previous && next === "OVERALL") {
      form.employeeTypeId = "";
      form.employeeTypeChildId = "";
    }
  },
...
watch(
  () => form.employeeTypeId,
  (next, previous) => {
    if (!hydratingForm.value && next !== previous)
      form.employeeTypeChildId = "";
  },
);
...
watch(
  () => filters.employeeTypeId,
  (next, previous) => {
    if (next !== previous) filters.employeeTypeChildId = "";
  },
);
...
<template>
  ...
  >
      ...
      >
        ...
        <template #filters>
          <EnterpriseFilterBar :loading="store.loading">
            <EnterpriseFilterField :label="t('common.search')" search wide>
            ...
            <EnterpriseFilterField :label="t('hrDashboardTarget.targetScope')">
              <Select
                v-model="filters.targetScope"
                :options="targetScopeFilterOptions"
                option-label="label"
                option-value="value"
              />
            </EnterpriseFilterField>
            ...
            <EnterpriseFilterField
              v-if="childFilterOptions.length > 1"
              :label="t('hrDashboardTarget.employeeTypeChild')"
            >
              <Select
    ...
    <template #cell-targetScope="{ row }">
      {{
        row.targetScope === "OVERALL"
          ? t("hrDashboardTarget.scopes.overall")
          : t("hrDashboardTarget.scopes.employeeType")
      }}
    </template>
    <template #cell-employeeType="{ row }">{{
      row.targetScope === "OVERALL"
        ? t("hrDashboardTarget.overallTarget")
        : row.employeeType?.name || "—"
    }}</template>
    <template #cell-employeeTypeChild="{ row }">{{
      row.employeeTypeChildName || "—"
    }}</template>
    <template #cell-targetRate="{ row }">
      <strong class="target-rate"
        >{{ Number(row.targetRate || 0).toFixed(2) }}%</strong
      >
    </template>
  ...
  >
    <div class="target-form">
      <div class="workspace-scope">
        <div>
          ...
          <strong>{{
            ...
            workspace.selectedCompany?.name ||
            "—"
          }}</strong>
        </div>
        <div>
      ...
      <label>
        <span>{{ t("hrDashboardTarget.targetScope") }} *</span>
        <Select
      ...
      <label v-if="formUsesEmployeeType">
      ...
      <label v-if="formUsesEmployeeType && formRequiresChild">
        <span>{{ t("hrDashboardTarget.employeeTypeChild") }} *</span>
        <Select
          v-model="form.employeeTypeChildId"
          :options="formChildOptions"
          option-label="label"
          option-value="value"
          filter
          :placeholder="t('hrDashboardTarget.selectEmployeeTypeChild')"
        />
      </label>
      <label>
...
.target-search :deep(.p-inputtext) {
  width: 100%;
  padding-left: 2rem;
}
.target-rate {
...
.target-form :deep(.p-inputtext),
.target-form :deep(.p-select),
.target-form :deep(.p-inputnumber),
.target-form :deep(.p-textarea) {
  width: 100%;
}
.workspace-scope {
  grid-column: 1 / -1;

...
import { useExcomStore } from "../stores/excom.store.js"
import { useWorkspaceStore } from "@/app/stores/workspace.store.js"


const { t } = useI18n()
const dashboardStore = useExcomStore()
...
const hasDashboardData = computed(() => {
    ...
    const turnoverHasData =
        (data.turnover?.rows || []).some((row) =>
            [row.previousCount, row.currentCount, row.previousRate, row.currentRate].some(
                (value) => Number(value) !== 0,
            ),
        ) ||
        Boolean((data.turnover?.byDepartment?.rows || []).length)
...
<template>
    <section class="excom-page hrms-compact">
        <div class="excom-page__sticky-tools">
            <DashboardFilterBar
                v-model="filters"
        ...
        <div class="excom-page__body">
            <Message
                v-if="dashboardStore.error"
                severity="error"
                :closable="false"
            >
                {{ safeT("excom.loadFailed", "Unable to load the HR dashboard.") }}
            </Message>
            ...
            <Message
                ...
                :closable="false"
            >
                {{ safeT("excom.noData", "No dashboard data matched this filter.") }}
            </Message>

            <div
                v-if="dashboardStore.loading && !dashboardStore.dashboard"
                class="excom-page__loading"
            >
                <ProgressSpinner />
            </div>
            ...
            <main
                v-else
                class="excom-page__content"
            >
                <article class="excom-module-card excom-module-card--general">
                    <GeneralDataSection
                ...
                <article class="excom-module-card">
                    <ManpowerSection
                        :title="safeT('excom.sections.manpower', 'Manpower')"
                ...
                <article class="excom-module-card">
                    <MovementSection
                        :title="safeT('excom.sections.movement', 'Movement')"
                        :subtitle="employeeTypeLabel"
                ...
                <article class="excom-module-card">
                    <AttendanceDashboardSection
                        :title="safeT('excom.sections.attendanceDashboard', 'Attendance Dashboard')"
                ...
                <article class="excom-module-card">
                    <TurnoverDashboardSection
                        :title="safeT('excom.sections.turnover', 'Turnover')"
                ...
                <article class="excom-module-card">
                    <RecruitmentChannelSection
                        :title="safeT('excom.sections.recruitmentChannels', 'Recruitment Channels')"
...
.excom-module-card--general {
    padding: 0.8rem;
}

.excom-module-card:hover {
...
.excom-page :deep(.dashboard-card),
.excom-page :deep(.dashboard-section),
.excom-page :deep(.p-datatable),
.excom-page :deep(.p-chart) {
    max-width: 100%;
}

/* The page card owns the outside edge. Child sections remain clean inside it. */
.excom-module-card :deep(> section),
.excom-module-card :deep(> div) {
    min-width: 0;
}

.excom-module-card :deep(.dashboard-section),
.excom-module-card :deep(.dashboard-card) {
    margin: 0;

...
const activeFilters = computed(
  () =>
    ...
    Number(query.dateFrom !== firstDay) +
    Number(query.dateTo !== today),
);
...
async function load(overrides = {}) {
  ...
  try {
    ...
    toast.add({
      severity: "error",
      summary: "Unable to load raw scans",
...
async function runImport() {
  ...
  try {
      ...
      onUploadProgress: (event) => {
        if (event.total)
          progress.value = Math.min(
            5,
            Math.round((event.loaded / event.total) * 5),
          );
      },
      onProgress: (job) => {
    ...
    toast.add({
      severity: "success",
      summary: "Raw scans imported",
      detail: `${summary.importedCount} new, ${summary.duplicateCount} duplicates.`,
      life: 3500,
    });
    await load({ page: 1 });
  } catch (error) {
...
<template>
    ...
    <template #controls>
      ...
      >
        <template #start>
          ...
          />
          <PermissionButton
            :permission="ATTENDANCE_PERMISSIONS.SCAN_IMPORT"
            label="Sample"
            icon="pi pi-download"
            severity="secondary"
            text
        ...
        <template #filters>
          <EnterpriseFilterBar :loading="loading">
            <EnterpriseFilterField class="scan-search" label="Search" search
              ><InputText
  ...
  >
    <div class="scan-import">
      <p>
        The backend validates every Employee ID against the active company and
        branch. If any row is invalid, nothing is saved.
      </p>
      <input
        type="file"
        accept=".xlsx,.xls"
      ...
      />
      <div v-if="importing" class="scan-progress">
        <div>
          <strong>{{ phase.replaceAll("_", " ") }}</strong
          ><span v-if="totalRows"
            >{{ processedRows }} / {{ totalRows }} rows</span
          >
        </div>

...
let timer
const leaveLabels={AL:"Annual Leave",ML:"Maternity Leave",SL:"Sick Leave",UL:"Unpaid Leave"}
const rows=computed(()=>store.items.map(item=>({...item,employeeName:item.employeeId?.displayName||"—",department:item.departmentId?.name||"—",position:item.positionId?.title||item.positionId?.name||"—",line:item.lineId?.name||"—",shift:item.shiftId?.name||item.shiftId?.code||"—",vacation:leaveLabels[item.leaveCode]||"—"})))
...
<AttendanceRecordDialog v-model:visible="dialogVisible" :form="form" :editing="Boolean(selectedId)" :saving="store.saving" :company-id="workspace.companyId" :branch-id="workspace.branchId" @save="save"/><AttendanceImportDialog v-model:visible="importVisible" :importing="store.importing" :progress="store.importProgress" :phase="store.importPhase" :processed-rows="store.importProcessedRows" :total-rows="store.importTotalRows" :result="store.importSummary" @file-change="selectedFile=$event" @template="store.downloadTemplate()" @import="importFile"/>

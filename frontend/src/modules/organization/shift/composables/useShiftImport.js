...
export function useShiftImport() {
    ...
    async function submit() {
        ...
        phaseMessageKey.value = "organization.shift.importPhaseUploading"
        processedRows.value = 0
        totalRows.value = 0
        result.value = null
        error.value = null
        ...
        try {
            const job = await startShiftImportJob(
                file.value,
                (event) => {
                    if (!event.total) {
                        return
            ...
            const completedJob = await waitForShiftImportJob(
                job.jobId,
                {
            ...
            if (completedJob.status === "FAILED") {
                const importError = new Error(
                    completedJob.error?.message || "Shift import failed.",
                )

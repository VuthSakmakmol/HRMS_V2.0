...
import {
    startPositionImportJob,
    waitForPositionImportJob,
...
export function usePositionImport() {
    ...
    async function submit() {
        ...
        phaseMessageKey.value = "organization.position.importPhaseUploading"
        processedRows.value = 0
        totalRows.value = 0
        result.value = null
        error.value = null
        ...
        try {
            const job = await startPositionImportJob(
                file.value,
                (event) => {
                    if (!event.total) {
                        return
            ...
            const completedJob = await waitForPositionImportJob(
                job.jobId,
                {
            ...
            if (completedJob.status === "FAILED") {
                const importError = new Error(
                    completedJob.error?.message || "Position import failed.",
                )

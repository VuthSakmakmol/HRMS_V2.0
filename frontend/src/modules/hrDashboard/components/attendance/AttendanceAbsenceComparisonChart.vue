...
const props = defineProps({
  ...
  targetRate: { type: Number, default: 4.88 },
  title: { type: String, default: "" },
  selectedPeriodKey: { type: String, default: null },
});
const { t } = useI18n();
const label = (r) =>
  r.month === "AVG" ? "AVG" : t(`hrDashboard.monthsShort.${r.month}`);
const chartData = computed(() => ({
  labels: props.rows.map(label),
  datasets: [
    {
      type: "bar",
...
<template>
  <EnterpriseChart :data="chartData" :title="title" percent :height="320" />
</template>

...
import {
    startLineImportJob,
    waitForLineImportJob,
} from "../api/line.api.js"

export function useLineImport() {
    const file = ref(null)
    ...
    async function submit() {
        ...
        phaseMessageKey.value = "organization.line.importPhaseUploading"
        processedRows.value = 0
        totalRows.value = 0
        result.value = null
        error.value = null
        ...
        try {
            const job = await startLineImportJob(
                file.value,
                (event) => {
                    if (!event.total) {
                        return
            ...
            const completedJob = await waitForLineImportJob(
                job.jobId,
                {
            ...
            if (completedJob.status === "FAILED") {
                const importError = new Error(
                    completedJob.error?.message || "Line import failed.",
                )

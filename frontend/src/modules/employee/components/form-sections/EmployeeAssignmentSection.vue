...
<template><div class="employee-section-grid">
  <label v-for="field in ['companyId','branchId','departmentId','positionId','lineId','shiftId']" :key="field" class="enterprise-form-field"><span>{{ ({companyId:'Company',branchId:'Branch',departmentId:'Department',positionId:'Position',lineId:'Line',shiftId:'Shift'})[field] }} *</span><Select v-model="form[field]" :options="options[field] || []" option-label="label" option-value="value" filter :disabled="disabled || (editing && ['companyId','branchId'].includes(field)) || (field==='branchId'&&!form.companyId) || (field==='departmentId'&&!form.branchId) || (['positionId','lineId'].includes(field)&&!form.departmentId) || (field==='shiftId'&&!form.branchId)" @change="field==='companyId'?$emit('company-change'):field==='branchId'?$emit('branch-change'):field==='departmentId'?$emit('department-change'):null"/><small v-if="errors[field]">{{ errors[field][0] || errors[field] }}</small></label>

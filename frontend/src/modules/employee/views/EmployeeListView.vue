...
const selectedColumns=ref(defaultEmployeeColumns()),lookups=reactive({companies:[],branches:[],departments:[],positions:[],lines:[],shifts:[],employeeTypes:[],recruitmentChannels:[],exitReasons:[]})
...
const formOptions=computed(()=>({
 companyId:map(lookups.companies),branchId:map(lookups.branches),departmentId:map(lookups.departments),positionId:map(lookups.positions),lineId:map(lookups.lines),shiftId:map(lookups.shifts),employeeTypeId:map(lookups.employeeTypes),recruitmentChannelId:map(lookups.recruitmentChannels),exitReasonId:map(lookups.exitReasons),introducerEmployeeId:[],approvalPolicyId:[],defaultRoleId:[],
}))
...
async function loadCompanyChildren(companyId){clearLookups('branches','departments','positions','lines','shifts','recruitmentChannels','exitReasons');lookups.employeeTypes=[];if(!companyId)return;const [branches,employeeTypes]=await Promise.all([fetchEmployeeBranches(companyId),fetchEmployeeTypes(companyId)]);lookups.branches=branches;lookups.employeeTypes=employeeTypes}
...
async function openCreateEmployee(){formState.openCreate();formState.form.companyId=workspace.companyId;formState.form.branchId=workspace.branchId;clearLookups('branches','departments','positions','lines','shifts','employeeTypes','recruitmentChannels','exitReasons');await loadCompanyChildren(workspace.companyId);formState.form.branchId=workspace.branchId;await loadBranchChildren()}

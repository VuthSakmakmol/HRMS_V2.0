...
async function loadBranchChildren(){const {companyId,branchId}=formState.form;clearLookups('departments','positions','lines','shifts','recruitmentChannels','exitReasons');if(!companyId||!branchId)return;const [departments,lines,shifts,recruitmentChannels,exitReasons]=await Promise.all([fetchEmployeeDepartments({companyId,branchId}),fetchEmployeeLines({companyId,branchId}),fetchEmployeeShifts({companyId,branchId}),fetchEmployeeRecruitmentChannels({companyId,branchId}),fetchEmployeeExitReasons({companyId,branchId})]);lookups.departments=departments;lookups.lines=lines;lookups.shifts=shifts;lookups.recruitmentChannels=recruitmentChannels;lookups.exitReasons=exitReasons}
async function loadDepartmentChildren(){const {companyId,branchId,departmentId}=formState.form;clearLookups('positions');if(!companyId||!branchId||!departmentId)return;lookups.positions=await fetchEmployeePositions({companyId,branchId,departmentId})}
...
async function onFormDepartmentChange(){formState.form.positionId='';await loadDepartmentChildren()}
...
<EmployeeImportDialog :visible="importState.visible.value" :importing="importState.importing.value" :progress="importState.progress.value" :phase="importState.phase.value" :phase-message-key="importState.phaseMessageKey.value" :processed-rows="importState.processedRows.value" :total-rows="importState.totalRows.value" :result="importState.result.value" @update:visible="importState.visible.value=$event" @file-change="importState.setFile" @download-template="downloadEmployeeImportTemplate" @import="submitImport"/>
</template>

...
import {
  ...
  listEmployeeTypePositionAssignmentsController,
...
router.get(
  ...
  validateRequest({ query: employeeTypeListQuerySchema }),
  asyncHandler(listEmployeeTypePositionAssignmentsController),
);
router.get(
  "/import-template",
  requirePermission(EMPLOYEE_TYPE_PERMISSIONS.VIEW),
  asyncHandler(downloadEmployeeTypeImportTemplateController),
);
router.get(
  "/export",
  requirePermission(EMPLOYEE_TYPE_PERMISSIONS.EXPORT),
  validateRequest({ query: employeeTypeListQuerySchema }),
  asyncHandler(exportEmployeeTypesController),
);
router.post(
  "/import-jobs",
  ...
  upload.single("file"),
  validateRequest({ query: employeeTypeListQuerySchema }),
  asyncHandler(startEmployeeTypeImportJobController),
);
router.get(
  "/import-jobs/:jobId",
  requirePermission(EMPLOYEE_TYPE_PERMISSIONS.IMPORT),
  asyncHandler(getEmployeeTypeImportJobController),
);
router.post(
  "/import",
  requirePermission(EMPLOYEE_TYPE_PERMISSIONS.IMPORT),
  upload.single("file"),
  validateRequest({ query: employeeTypeListQuerySchema }),
  asyncHandler(importEmployeeTypesController),
);
router.get(
  "/",
  requirePermission(EMPLOYEE_TYPE_PERMISSIONS.VIEW),
  validateRequest({ query: employeeTypeListQuerySchema }),
  asyncHandler(listEmployeeTypesController),
);
router.post(
  "/",
  requirePermission(EMPLOYEE_TYPE_PERMISSIONS.CREATE),
  validateRequest({ body: employeeTypeCreateSchema }),
  asyncHandler(createEmployeeTypeController),
);
router.get(
  "/:employeeTypeId",
  requirePermission(EMPLOYEE_TYPE_PERMISSIONS.VIEW),
  validateRequest({ params: employeeTypeIdParamSchema }),
  asyncHandler(getEmployeeTypeController),
);
router.patch(
  "/:employeeTypeId",
  requirePermission(EMPLOYEE_TYPE_PERMISSIONS.UPDATE),
  validateRequest({
    params: employeeTypeIdParamSchema,
    body: employeeTypeUpdateSchema,
  }),
  asyncHandler(updateEmployeeTypeController),
);
router.patch(
  "/:employeeTypeId/archive",
  requirePermission(EMPLOYEE_TYPE_PERMISSIONS.ARCHIVE),
  validateRequest({ params: employeeTypeIdParamSchema }),
  asyncHandler(archiveEmployeeTypeController),
);

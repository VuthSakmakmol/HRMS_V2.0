...
async function persistAllPositionAssignment({ employeeType, positionId, session }) {
    const normalizedPositionId = toId(positionId)
    if (!normalizedPositionId) return

    if (employeeType.positionAssignmentMode === "ALL_POSITIONS") {
        if (!(employeeType.positionIds || []).some((item) => sameId(item, positionId))) {
    ...
    if ((allPositionsChild.positionIds || []).some((item) => sameId(item, positionId))) return

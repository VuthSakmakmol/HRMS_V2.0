...
export async function createEmployeeController(req, res) {
  ...
  return sendSuccess(req, res, {
    statusCode: 201,
    data: { employee },
    messageKey: "messages.employee.profile.created",
  });
}
...
export async function updateEmployeeController(req, res) {
  ...
  return sendSuccess(req, res, {
    data: { employee },
    messageKey: "messages.employee.profile.updated",
  });
}
...
export async function archiveEmployeeController(req, res) {
  ...
  return sendSuccess(req, res, {
    data: { employee },
    messageKey: "messages.employee.profile.archived",
  });
}

export async function getEmployeeApprovalPreviewController(req, res) {
  const preview = await getEmployeeApprovalPreview({
    query: req.validatedQuery,
    user: req.auth.user,
  });
...
export async function exportEmployeesController(req, res) {
  const employees = await getExportEmployees({
    query: req.validatedQuery,
    user: req.auth.user,
  });
...
async function processEmployeeImportJob({
  jobId,
  fileBuffer,
  context,
  user,
  ...
  try {
    updateImportJob(jobId, {
      ...
      phase: "READING_FILE",
      percent: 8,
      messageKey: "errors.employee.import.phaseReading",
    });
    ...
    updateImportJob(jobId, {
      ...
      messageKey: "errors.employee.import.phaseValidating",
    });
    const summary = await importEmployeesFromRows({
      ...
      onProgress(progress) {
        updateImportJob(jobId, {
          status: "PROCESSING",
          messageKey:
            progress.phase === "SAVING_ROWS"
              ? "errors.employee.import.phaseSaving"
              : "errors.employee.import.phaseResolving",
          ...progress,
        });
      },
    });
    ...
    updateImportJob(jobId, {
      messageKey: "errors.employee.import.phaseCompleted",
    });
  } catch (error) {
    failImportJob(jobId, error);
    updateImportJob(jobId, {
      messageKey: "errors.employee.import.phaseFailed",
    });
  }
}
...
export async function startEmployeeImportJobController(req, res) {
  if (!req.file) {
    throw new AppError({
      statusCode: 422,
      code: "EMPLOYEE_IMPORT_FILE_REQUIRED",
      messageKey: "errors.employee.profile.importFileRequired",
      fields: { file: ["errors.employee.profile.importFileRequired"] },
    });
  }
  ...
  updateImportJob(job.jobId, {
    percent: 5,
    phase: "UPLOADED",
    messageKey: "errors.employee.import.phaseUploaded",
  });
  ...
  return sendSuccess(req, res, {
    statusCode: 202,
    data: { job: queuedJob },
    messageKey: "errors.employee.import.started",
  });
}
...
export async function getEmployeeImportJobController(req, res) {
  ...
  if (!job)
    throw new AppError({
      statusCode: 404,
      code: "EMPLOYEE_IMPORT_JOB_NOT_FOUND",
      messageKey: "errors.employee.import.jobNotFound",
    });

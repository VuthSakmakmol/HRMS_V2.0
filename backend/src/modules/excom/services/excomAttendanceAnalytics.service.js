...
import { endOfBusinessDay, getBusinessTimeZone, startOfBusinessDay } from "../../attendance/utils/attendanceDate.util.js"
...
const ABSENCE_OVERALL_TYPES = Object.freeze([
    { code: "UL", label: "Unpaid Leave", showDay: true, group: "absence" },
...
function shiftDateKeyByYears(value, yearOffset) {
...
function attendanceRangesFromPeriods(periods = [], yearOffset = 0) {
    return periods
        .map((period) => {
            const startKey = dateKeyFromUtcDate(period?.startDate)
            const endKey = dateKeyFromUtcDate(period?.endDate)
            if (!startKey || !endKey) return null

            const shiftedStartKey = yearOffset
                ? shiftDateKeyByYears(startKey, yearOffset)
                : startKey
            const shiftedEndKey = yearOffset
                ? shiftDateKeyByYears(endKey, yearOffset)
                : endKey

            if (!shiftedStartKey || !shiftedEndKey) return null

            return {
                attendanceDate: {
                    $gte: startOfBusinessDay(shiftedStartKey),
                    $lte: endOfBusinessDay(shiftedEndKey),
                },
            }
        })
        .filter(Boolean)
...
function buildAttendanceMatch(query, selectedYear, employees = [], startDate, endDate, periods = []) {
    // Excom always presents Jan-Dec. The focused month keeps the exact selected
    // days, while all other months use their complete month. Build the Mongo
    // date match from those 12 presentation periods using the same Cambodia
    // business-day conversion as Attendance Records. The previous-year ranges
    // mirror the same day boundaries for a fair year-on-year comparison.
    let currentRanges = attendanceRangesFromPeriods(periods, 0)
    let previousRanges = attendanceRangesFromPeriods(periods, -1)

    // Backward-compatible fallback for callers without presentation periods.
    if (!currentRanges.length) {
        const selectedStartKey = query.startDate || startDate?.toISOString?.().slice(0, 10) || `${selectedYear}-01-01`
        const selectedEndKey = query.endDate || endDate?.toISOString?.().slice(0, 10) || `${selectedYear}-12-31`
        const previousStartKey = shiftDateKeyByYears(selectedStartKey, -1)
        const previousEndKey = shiftDateKeyByYears(selectedEndKey, -1)

        currentRanges = [{
            attendanceDate: {
                $gte: startOfBusinessDay(selectedStartKey),
                $lte: endOfBusinessDay(selectedEndKey),
            },
        }]
        previousRanges = [{
            attendanceDate: {
                $gte: startOfBusinessDay(previousStartKey),
                $lte: endOfBusinessDay(previousEndKey),
            },
        }]
    }

    const match = {
        $or: [...previousRanges, ...currentRanges],
    }
...
async function aggregateAttendance(query, selectedYear, employees, startDate, endDate, periods = []) {
    const match = buildAttendanceMatch(query, selectedYear, employees, startDate, endDate, periods)
    ...
    const [result = {}] = await AttendanceRecord.aggregate([
        ...
        },
        {
            $set: {
                year: {
                    $year: { date: "$attendanceDate", timezone: BUSINESS_TIME_ZONE },
                },
                month: {
                    $month: { date: "$attendanceDate", timezone: BUSINESS_TIME_ZONE },
                },
...
export async function buildExcomAttendanceAnalytics({
    query,
    employees,
    selectedYear,
    startDate,
    ...
    const groups = await aggregateAttendance(query, selectedYear, employees, startDate, endDate, periods)

...
import { buildExcomAttendanceAnalytics } from "./excomAttendanceAnalytics.service.js"
...
function createPeriods(startDate, endDate) {
    // Excom date selection is a FOCUS window, not a request to hide the
    // remaining months. Always return Jan-Dec of the selected (end-date) year.
    // Months outside the focus keep their complete calendar-month range.
    // Months touched by the focus use the exact selected day boundaries.
    // Example: 01-Jul -> 15-Jul keeps Jan-Dec visible, while July is calculated
    // from only 01-Jul through 15-Jul and is highlighted by selectedPeriodKey.
    const selectedYear = endDate.getUTCFullYear()
    const yearStart = monthStart(selectedYear, 1)
    const yearEnd = monthEnd(selectedYear, 12)
    const focusStart = new Date(Math.max(startDate, yearStart))
    const focusEnd = new Date(Math.min(endDate, yearEnd))
    const hasFocusInSelectedYear = focusStart <= focusEnd

    return Array.from({ length: 12 }, (_, index) => {
        const month = index + 1
        const fullStartDate = monthStart(selectedYear, month)
        const fullEndDate = monthEnd(selectedYear, month)
        const isFocused = hasFocusInSelectedYear &&
            fullStartDate <= focusEnd &&
            fullEndDate >= focusStart
        const periodStartDate = isFocused
            ? new Date(Math.max(fullStartDate, focusStart))
            : fullStartDate
        const periodEndDate = isFocused
            ? new Date(Math.min(fullEndDate, focusEnd))
            : fullEndDate
        ...
        return {
            ...
            fullEndDate,
            isFocused,
            isPartialFocus: isFocused && (
                periodStartDate.getTime() !== fullStartDate.getTime() ||
                periodEndDate.getTime() !== fullEndDate.getTime()
            ),
            budget: 0,
            roadmap: 0,
            actual: 0,
...
function dateMatchesPeriods(date, periods = []) {
...
async function loadEmployees(query) {
    const cacheKey = `excom:source:employees:${JSON.stringify(normalizedQuery(query))}`
...
async function loadPlans(query, periods) {
    ...
    const cacheKey = `excom:source:plans:${JSON.stringify({
...
async function loadMovements(query, startDate, endDate) {
    ...
    const cacheKey = `excom:source:movements:${JSON.stringify({
        query: normalizedQuery(query),
...
async function loadRecruitmentChannels(query = {}) {
    ...
    const cacheKey = `excom:source:recruitment:${JSON.stringify({
...
async function loadDashboardTargets(query, year) {
    ...
    const cacheKey = `excom:source:targets:${JSON.stringify({
        companyId: query.companyId || null,
        branchId: query.branchId || null,
        year,
    })}`
...
function buildRecruitmentChannelDashboard({
    ...
    const previousPeriods = periods.map((period) => ({
        ...period,
        key: `${period.year - 1}-${String(period.month).padStart(2, "0")}`,
        year: period.year - 1,
        startDate: shiftDateByYears(period.startDate, -1),
        endDate: shiftDateByYears(period.endDate, -1),
    }))
    const rows = []
    ...
    for (const employee of employees) {
        ...
        const previousPeriod = previousPeriods.find((period) =>
            dateIsWithinRange(joinDate, period.startDate, period.endDate),
        )
        ...
        if (currentPeriod) {
            const monthIndex = monthIndexByKey.get(currentPeriod.key)
    ...
    return {
        ...
        periods: periods.map((period) => ({
            ...
            month: period.month,
            label: period.label,
            isFocused: Boolean(period.isFocused),
            isPartialFocus: Boolean(period.isPartialFocus),
        })),
        rows: sortedRows,
...
function buildTurnoverByDepartment({
    employees = [],
    periods = [],
    selectedYear,
    departments = [],
    ...
    const currentYearPeriods = (periods || [])
        .filter((period) => Number(period.year) === Number(selectedYear))
        .map((period) => ({
            ...createTurnoverComparisonMonth({
                year: period.year,
                month: period.month,
    ...
    const departmentById = new Map(
        (departments || []).map((department) => [
            String(department.id || department._id?.toString?.() || ""),
            department,
        ]),
    )
    ...
    for (const employee of employees || []) {
        const departmentId = employee.departmentId?.toString?.() || "UNASSIGNED"

        if (!employeesByDepartment.has(departmentId)) {
            employeesByDepartment.set(departmentId, [])
        }

        employeesByDepartment.get(departmentId).push(employee)
    }

    const rows = []

    for (const [departmentId, departmentEmployees] of employeesByDepartment.entries()) {
        const department = departmentById.get(departmentId)
        const months = currentYearPeriods.map((period) => ({
            ...createTurnoverComparisonMonth({
                year: period.year,
                month: period.month,
            }),
        ...
        for (const month of months) {
            ...
            month.headcountStart = departmentEmployees.filter((employee) =>
                employeeWasActiveOn(employee, month.startDate),
            ).length
            month.headcountEnd = departmentEmployees.filter((employee) =>
                employeeWasActiveOn(employee, month.endDate),
            ).length
        }
        ...
        for (const employee of departmentEmployees) {
            ...
            if (Number.isNaN(resignDate.getTime())) continue

            const month = months.find((period) =>
                dateIsWithinRange(resignDate, period.startDate, period.endDate),
            )

            if (month) month.exits += 1
        }
        ...
        rows.push({
            ...
            departmentName:
                ...
                department?.label ||
                department?.code ||
                (departmentId === "UNASSIGNED" ? "Unassigned" : "Unknown Department"),
            label:
                department?.name ||
                department?.label ||
                department?.code ||
                (departmentId === "UNASSIGNED" ? "Unassigned" : "Unknown Department"),
            exits: summary.exits,
            averageHeadcount: summary.averageHeadcount,
            turnoverRate: summary.rate,
            months: months.map((month) => ({
                ...
                label: month.label,
                exits: month.exits,
                headcountStart: month.headcountStart,
                headcountEnd: month.headcountEnd,
                averageHeadcount: month.averageHeadcount,
                turnoverRate: month.rate,
            })),
        })
    }
    ...
    rows.sort(
        (a, b) =>
            Number(b.turnoverRate || 0) - Number(a.turnoverRate || 0) ||
            Number(b.exits || 0) - Number(a.exits || 0) ||
            String(a.departmentName || a.label || "").localeCompare(
                String(b.departmentName || b.label || ""),
            ),
    )
    ...
    const rows = parentEmployeeTypes
        .map((option) => {
            ...
            return {
                ...
                // This is intentionally user-maintained display text. Excom
...
function buildExitAnalysisDashboard({
    ...
    const allExitEmployees = employees.filter((employee) => {
        ...
        const resignDate = new Date(employee.resignDate)
        return dateMatchesPeriods(resignDate, periods)
    })
...
function buildWorkforceRatioSummary({ employees = [], setups = [], selectedDate }) {
    ...
    // Company + Branch. If Excom is ever opened at a multi-branch scope, only
...
async function loadExitReasons(query = {}) {
    const cacheKey = `excom:source:exit-reasons:${JSON.stringify({
        companyId: query.companyId || null,
        branchId: query.branchId || null,
    })}`
    const cached = getCache(cacheKey)
...
export async function getExcomLookups({ query }) {
    const cacheKey = `excom:lookups:${JSON.stringify(query)}`
...
function buildDataReadiness({ employees, plans, attendanceRecordCount = 0, recruitmentChannels, dashboardTargets, workforceRatioSetupCount = 0 }) {
    ...
    const items = [
        ...
        { key: "targets", label: "Excom Targets", ready: dashboardTargets.length > 0, count: dashboardTargets.length, detail: dashboardTargets.length ? `${dashboardTargets.length.toLocaleString()} targets configured` : "No Excom targets configured" },
    ]
...
export async function getExcom({ query }) {
    const cleanQuery = normalizedQuery(query)
    const { forceRefresh = false, ...cacheableQuery } = cleanQuery
    const cacheKey = `excom:data:${JSON.stringify(cacheableQuery)}`
    ...
    if (
        ...
        throw new Error("Invalid Excom date range")
    }

    const selectedYear = selectedEndDate.getUTCFullYear()

    // Excom date selection is a presentation focus. Jan-Dec always remains
    // visible for the selected year. The focused month(s) use the exact day
    // boundaries from the filter (for example 01-Jul -> 15-Jul), while every
    // other month keeps its complete month data for easy comparison.
    const startDate = selectedStartDate
    const endDate = selectedEndDate
    const periods = createPeriods(startDate, endDate)
    const isWholeYearFocus =
        startDate.getUTCFullYear() === selectedYear &&
        endDate.getUTCFullYear() === selectedYear &&
        startDate.getUTCMonth() === 0 &&
        startDate.getUTCDate() === 1 &&
        endDate.getUTCMonth() === 11 &&
        endDate.getUTCDate() === 31
    const selectedPeriodKey = isWholeYearFocus
        ? null
        : `${selectedYear}-${String(selectedEndDate.getUTCMonth() + 1).padStart(2, "0")}`

    const lookupsPromise = getExcomLookups({
        query: {
    ...
    turnoverComparison.byDepartment = buildTurnoverByDepartment({
        employees,
        periods,
        selectedYear,
        departments: lookups.departments,
    ...
    const result = {
        filters: {
            ...
            focusStartDate: cleanQuery.startDate,
            focusEndDate: cleanQuery.endDate,
            wholeYearFocus: isWholeYearFocus,
